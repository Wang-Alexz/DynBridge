...
from data_generation.flow_utils import sample_tracks_visible_first, sample_batch_tracks_visible_first, sample_double_grid, sample_fixbatch_tracks
...
class Actor(nn.Module):
    def __init__(
        ...
        self._track_token = nn.Parameter(torch.randn(1, 1, 2*self.num_token, repr_dim))
        # GPT model
        if policy_type == "gpt":
            self._policy = GPT(
                GPTConfig(
        ...
        self.alpha = alpha
        self.anchor = nn.Parameter(torch.randn(8, hidden_dim*2))
        self.track_decoder = nn.Linear(hidden_dim*2, 2*int(self.track_ts*self.num_point/self.num_token))
        self.transtrack_decoder = nn.Linear(2*self.num_token, 2) 
        self.track_policy = nn.MultiheadAttention(embed_dim=hidden_dim*2, num_heads=8, batch_first=True)
        self.cls_token = nn.Parameter(torch.randn(1, 1, 2, hidden_dim*2))

        self.Wq = nn.Linear(hidden_dim*2, hidden_dim*2)
        self.Wk = nn.Linear(hidden_dim*2, hidden_dim*2)
        self.Wv = nn.Linear(hidden_dim*2, hidden_dim*2)
        self.adapter_down = nn.Linear(hidden_dim*2, hidden_dim)
        self.adapter_up = nn.Linear(hidden_dim, hidden_dim*2)
        self.act = nn.ReLU()
        self.apply(utils.weight_init)

    def adapt_fuse(self, track_features):
        """
        track_features: [B, T, N, D]
        return: [B, T, num_cls, D]
        """
        B, T, N, D = track_features.shape

        cls = self._action_token.expand(B, T, 2, D).detach()   # [B, T, 2, D]
        cls = self.adapter_up(self.act(self.adapter_down(cls)))

        Q = self.Wq(cls)                  # [B, T, 2, D]
        K = self.Wk(track_features)       # [B, T, N, D]
        V = self.Wv(track_features)       # [B, T, N, D]

        attn = torch.einsum("btmd,btnd->btmn", Q, K) / (D ** 0.5)
        attn = F.softmax(attn, dim=-1)

        attn_fused = torch.einsum("btmn,btnd->btmd", attn, V)

        transtrack_features = self.transtrack_decoder(track_features.transpose(2, 3)).transpose(2, 3) 

        fused = self.alpha * attn_fused + (1 - self.alpha) * transtrack_features
        return fused

    def forward(self, obs, num_prompt_feats, stddev, alltracks=None, action=None, cluster_centers=None):
        B, T, D = obs.shape
        history_len = (T-num_prompt_feats) // self._num_feat_per_step
        if alltracks is not None: track_truth = alltracks[:, -history_len:, 1:self.track_ts+1].float()
        ...
        if self._policy_type == "mlp":
            ...
            track_obs = obs[:, num_prompt_feats:]
            track_obs = track_obs.view(B, -1, self._num_feat_per_step, obs.shape[-1])
            enc_track_concat = self._track_token.repeat(B, track_obs.shape[1], 1, 1)

            Q = enc_track_concat.view(B, -1, D)       
            K1 = track_obs.view(B, -1, D)       
            K2 = prompt                               
            memory = torch.cat([K1, K2], dim=1)        
            features, _ = self.track_policy(Q, memory, memory)   
            track_features = features.view(B, history_len, -1, D)

            track_reconstruct = self.track_decoder(track_features).reshape(B*2, -1, self.track_ts, self.num_point, 2)
            if alltracks is not None: flow_loss = self.flow_weight * F.mse_loss(track_reconstruct, track_truth)
            transtrack_features = self.adapt_fuse(track_features)
            ...
            obs = torch.cat([obs, transtrack_features, action_token], dim=-2).view(B, -1, D)
            obs = torch.cat([prompt, obs], dim=1)

            # get action features
            features = self._policy(obs)
            features = features[:, num_prompt_feats:]
            num_feat_per_step = self._num_feat_per_step + 3  
...
class BCAgent:
    ...
    def act(self, obs, prompt, norm_stats, step, global_step, eval_mode=False):
        ...
        if self.use_language:
            ...
            lang_features = (
                torch.as_tensor(prompt["task_emb"], device=self.device).squeeze(0)
        ...
        if self.obs_type == "pixels":
            ...
            for key in self.pixel_keys:
                ...
                img_pixels = self.customAug(pixels / 255.0) if self.norm else pixels
                # encoder
                lang = lang_features if self.film else None
                pixels = (
                    self.encoder[key](img_pixels, lang=lang)
                    if self.separate_encoders
                    else self.encoder(img_pixels, lang=lang)
                )
    ...
    def update(self, expert_replay_iter, step, update=True):
        ...
        tracks, vis = data['tracks'], data['vis']
        egotracks, egovis = data['egotracks'], data['egovis']
        ...
        if self.obs_type == "pixels":
            features = []
            alltracks = torch.cat([tracks, egotracks], dim=0) #[tracks, egotracks]
            for key in self.pixel_keys:
        ...
        if update:
            ...
            _, actor_loss = self.actor(
                features, num_prompt_feats, stddev, alltracks, action, **kwargs
            )
