...
class BCDataset(IterableDataset):
    ...
    def _sample(self):
        ...
        task_emb = episodes["task_emb"].squeeze(0).detach()
        if observations["gripper_states"].shape[-1] != 2:
            # 将平均值复制成 2 维，以匹配 Franka 两指
            # print('xxxxxxxxxxxxxxxxxxxxxxxx')
            gripper_mean = np.mean(observations["gripper_states"], axis=1)
            observations["gripper_states"] = np.repeat(gripper_mean[:, None], 2, axis=1)

        if self._obs_type == "pixels":
            # Sample obs, action
            last_frame, last_track, last_vis = observations["pixels"][-1:], observations["tracks"][-1:], observations["vis"][-1:]
            vids = np.concatenate([observations["pixels"], np.repeat(last_frame, self.num_track_ts, axis=0)], axis=0)
            tracks = np.concatenate([observations["tracks"], np.repeat(last_track, self.num_track_ts, axis=0)], axis=0)
            vis = np.concatenate([observations["vis"], np.repeat(last_vis, self.num_track_ts, axis=0)], axis=0)

            last_egoframe, last_egotrack, last_egovis = observations["pixels_egocentric"][-1:], observations["egotracks"][-1:], observations["egovis"][-1:]
            egovids = np.concatenate([observations["pixels_egocentric"], np.repeat(last_egoframe, self.num_track_ts, axis=0)], axis=0)
            egotracks = np.concatenate([observations["egotracks"], np.repeat(last_egotrack, self.num_track_ts, axis=0)], axis=0)
            egovis = np.concatenate([observations["egovis"], np.repeat(last_egovis, self.num_track_ts, axis=0)], axis=0)
            ...
            next_sampled_pixel = observations["pixels"][
                sample_idx + 1 : sample_idx + self._history_len + 1
            ]
            next_sampled_pixel_egocentric = observations["pixels_egocentric"][
                sample_idx + 1 : sample_idx + self._history_len + 1
            ]
            sampled_egotracks = egotracks[
                sample_idx : sample_idx + self._history_len + self.num_track_ts
            ]
            sampled_egovis = egovis[
                sample_idx : sample_idx + self._history_len + self.num_track_ts
            ]
            ...
            next_sampled_pixel = torch.stack(
                [self.aug(next_sampled_pixel[i]) for i in range(len(next_sampled_pixel))]
            )
            next_sampled_pixel_egocentric = torch.stack(
                [
                    self.aug(next_sampled_pixel_egocentric[i])
                    for i in range(len(next_sampled_pixel_egocentric))
                ]
            )

            sampled_tracks = torch.stack(
                [torch.tensor(sampled_tracks[i]) for i in range(len(sampled_tracks))]
            )
            sampled_vis = torch.stack(
                [torch.tensor(sampled_vis[i]) for i in range(len(sampled_vis))]
            )            
            sampled_egotracks = torch.stack(
                [torch.tensor(sampled_egotracks[i]) for i in range(len(sampled_egotracks))]
            )            
            sampled_egovis = torch.stack(
                [torch.tensor(sampled_egovis[i]) for i in range(len(sampled_egovis))]
            )
            ...
            next_sampled_proprioceptive_state = np.concatenate(
                [
                    observations["joint_states"][
                        sample_idx + 1 : sample_idx + self._history_len + 1
                    ],
            ...
            if self._prompt == "text":
                return {
                    ...
                    "next_pixels": next_sampled_pixel,
                    "next_pixels_egocentric": next_sampled_pixel_egocentric,
                    "tracks": sampled_tracks,
                    "vis": sampled_vis,
                    "egotracks": sampled_egotracks,
                    "egovis": sampled_egovis,
                    "proprioceptive": self.preprocess["proprioceptive"](
                        sampled_proprioceptive_state
                    ),
                    "next_proprioceptive": self.preprocess["proprioceptive"](
                        next_sampled_proprioceptive_state
                    ),
