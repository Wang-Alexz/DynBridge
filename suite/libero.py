...
sentence_encoder = AutoModel.from_pretrained("all-MiniLM-L6-v2")
...
class RGBArrayAsObservationWrapper(dm_env.Environment):
    ...
    def __init__(
        ...
        # task emb
        inputs = tokenizer(self._env.language_instruction, return_tensors='pt', padding=True, truncation=True)
        last_hidden_state = sentence_encoder(**inputs).last_hidden_state
        self.task_emb = last_hidden_state[:, 0, :]  # 选取 [CLS] token 的隐藏状态作为文本的表示
