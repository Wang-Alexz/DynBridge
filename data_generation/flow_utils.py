...
def sample_batch_tracks_visible_first(tracks, vis, num_samples=16):
    """
    Sample visible points on the initial frame for each item in the batch.
    ...
    Returns:
        sampled_tracks: (B, T, num_samples, 2)
        sampled_vis: (B, T, num_samples)
    """
    ...
    for b in range(B):
        vis_idx = torch.where(vis[b, 0] > 0)[0]
        if len(vis_idx) == 0:
            raise ValueError(f"No visible points in batch item {b} at time 0")

        # 随机采样 num_samples 个 index（带 replacement 以防数量不足）
        idx = torch.randint(0, len(vis_idx), (num_samples,), device=tracks.device)
        selected_idx = vis_idx[idx]

        sampled_tracks_list.append(tracks[b, :, selected_idx])  # (T, num_samples, 2)
        sampled_vis_list.append(vis[b, :, selected_idx])        # (T, num_samples)

    sampled_tracks = torch.stack(sampled_tracks_list, dim=0)  # (B, T, num_samples, 2)
    sampled_vis = torch.stack(sampled_vis_list, dim=0)        # (B, T, num_samples)
    return sampled_tracks, sampled_vis

def sample_fixbatch_tracks(tracks, vis, num_samples=16):


    B, T, N, _ = tracks.shape

    if num_samples > N:
        raise ValueError(f"num_samples ({num_samples}) cannot be greater than number of points ({N})")

    fixed_idx = torch.arange(num_samples, device=tracks.device)  # (num_samples,)

    # 用 index_select 批量采样
    sampled_tracks = tracks[:, :, fixed_idx, :]  # (B, T, num_samples, 2)
    sampled_vis = vis[:, :, fixed_idx]           # (B, T, num_samples)
